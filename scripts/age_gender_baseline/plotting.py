...
inspected for overfitting, per RV-004's acceptance criteria.
"""
...
def save_loss_curves(curves: dict, task_name: str, out_path: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("value")
    ax.set_title(f"{task_name} baseline: loss & accuracy curves")
    ax.legend()
    fig.tight_layout()
