...
CLI entry point for RV-006: runs one live-camera evaluation session for a
single condition (e.g. normal lighting, low light, face turned away,
angled face), classifying detected faces with the RV-005 fine-tuned
age/gender models and logging per-frame predictions against a supplied
ground truth. Run this once per condition you want to test, then run
scripts/summarize_real_world_eval.py to aggregate accuracy per condition.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="RV-006 real-world evaluation session")
